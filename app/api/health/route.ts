...
export async function GET() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          try {
            ...
            msg = data?.error?.message || res.statusText;
          } catch {
            msg = res.statusText;
          }
          services.openai = { ok: false, model, error: msg };
        }
      } catch (err) {
        services.openai = { ok: false, model, error: (err as Error).message };
      }
    }
  }
  ...
  // Overall status
  const status = Object.values(services).every((svc) => svc.ok) ? 'ok' : 'error';
